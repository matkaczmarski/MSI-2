...
\indent\indent 		$m_i = x_{r_1} + F \cdot  (x_{r_2} - x_{r_3})$\\
\indent\indent		\textcolor{blue}{for} j = 1 to D\\
\indent\indent		\{\\
\indent\indent\indent			wybierz losowo $j_{rand}$ z przedziału $[1, D]$\\
\indent\indent\indent			\textcolor{blue}{if} (rand() $<$ CR or j $==$ $j_{rand}$)\\
\indent\indent\indent\indent				u[j] = $m_i$[j]\\
\indent\indent\indent			\textcolor{blue}{else}\\
\indent\indent\indent\indent				u[j] = $x_i$[j]\\
\indent\indent		\}\\
